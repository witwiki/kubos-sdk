...
def test_bash_called_with_correct_args_dfu_util():
    ...
    subprocess.check_call.assert_called_with(['/bin/bash', 'flash/dfu_util/flash.sh', 'bin/osx/dfu-util', ''])

def test_bash_called_with_correct_args_openocd():
    flash.flash_openocd("", "")
    subprocess.check_call.assert_called_with(['/bin/bash', 'flash/openocd/flash.sh', 'bin/osx/openocd', 'stm32f4_flash ', 'flash/openocd'])
